...
export const aiConfigService = {
  ...
  async saveMetaPixelId(pixelId: string): Promise<void> {
    ...
    const { error } = await supabase
      .from('system_configs')
      .upsert(
        { key: PIXEL_KEY, value: { id: clean }, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    if (error) throw error;
  },
  ...
  async saveMetaCapiToken(token: string): Promise<void> {
    ...
    const { error } = await supabase
      .from('system_configs')
      .upsert(
        { key: CAPI_KEY, value: { token: clean }, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    if (error) throw error;
  },
  ...
  async saveMetaTestEventCode(code: string): Promise<void> {
    ...
    const { error } = await supabase
      .from('system_configs')
      .upsert(
        { key: TEST_CODE_KEY, value: { code: clean }, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    if (error) throw error;
  },
