  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-purple-200 dark:border-zinc-800 rounded-2xl md:rounded-[2rem] p-5 sm:p-8 md:p-12 shadow-xl shadow-purple-500/5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 md:mb-10 gap-6">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-center lg:justify-end gap-2 sm:gap-3 w-full lg:w-auto">
          {activeTab === 'users' && (
            <>
              <Button
                variant="outline"
                onClick={handleExportContacts}
                className="w-full sm:w-auto rounded-xl border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800"
              >
                <Download size={18} className="mr-2 shrink-0" />
                Exportar
              </Button>
              <Button onClick={() => setIsUserDialogOpen(true)} className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white rounded-xl shadow-lg shadow-purple-500/20 transition-all hover:scale-[1.02]">
          ...
          <Button
            variant="outline"
            onClick={onBack}
            className={`w-full sm:w-auto rounded-xl border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 ${activeTab === 'users' ? 'col-span-2 sm:col-auto' : ''}`}
          >
