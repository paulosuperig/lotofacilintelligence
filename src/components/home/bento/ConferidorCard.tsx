...
interface ConferidorCardProps {
  onNavigate: () => void;
}

export const ConferidorCard = ({ onNavigate }: ConferidorCardProps) => (
  <div
    onClick={onNavigate}
    role="button"
    tabIndex={0}
    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onNavigate(); }}
    aria-label="Abrir o conferidor de jogos"
  ...
  >
    ...
    <div className="relative z-10">
      ...
      <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1">Conferidor</p>
      <h4 className="text-sm xs:text-base md:text-xl font-display font-bold leading-tight text-zinc-900 dark:text-zinc-100 mb-4">
        Confira contra qualquer concurso
      </h4>
    </div>
